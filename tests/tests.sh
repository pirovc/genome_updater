...
diff <(find "${out_direct}"/v1/files -type "f,l" -printf "%f\n" | sort) <(find "${out_mod}"/v2/files -type "f,l" -printf "%f\n" | sort )
...
diff <(find "${out_all}" -type "f,l" -printf "%f\n" | sort) <(find "${out_1}" "${out_2}" -type "f,l" -printf "%f\n" | sort)
